...
- **목표 설계 (Target)** — [PRD_v2.md](./PRD_v2.md)에 기술된 LangGraph 다단계 파이프라인(질문 분석 → 쿼리 최적화 → 검색 → 재랭킹 → 신뢰도 평가/재시도 → 답변 생성).
- **현재 구현 (Phase 1)** — `assistant/app/` 아래 실제로 동작 중인 코드. `docs/prd-assistant.md`(최초 설계안)을 바탕으로 구현됐으며, PRD_v2보다 단순한 그래프다. **Qdrant 컬렉션 2개(`wiki_summary`, `wiki_chunk`)를 순차 검색해 문서 단위로 병합하는 방식**이 이 Phase의 핵심이며, 여기에 더해 **신뢰도 게이트(RAGAS 스타일)와 쿼리 재작성 재시도 루프가 구현 완료**됐다(로드맵 1~2단계, 6장 참고).

두 층은 코드 구조·프로바이더·재랭킹 로직 등에서 여전히 차이가 있다. 아래 4장이 "현재 실제로 동작하는 것", 5장이 "PRD_v2가 그리는 목표", 6장이 그 사이의 격차와 진행 상황이다. **지금 코드를 수정하거나 API를 호출할 때는 반드시 4장을 기준으로 삼을 것.**
...
- **신뢰도 기반 재시도**: 검색 결과의 신뢰도(similarity + RAGAS 스타일 LLM 평가)를 자체 평가해 기준 미달 시 최대 `MAX_RETRIES`(기본 2)회까지 검색어를 재작성하며 재검색한다.
...
START
  → retrieve_summary
  → retrieve_chunk
  → route_after_hits ──(조건부 라우팅)──┬─→ build_context → confidence_checker ──(조건부 라우팅)──┬─→ generate_answer → extract_sources → END
                                        │                                                        ├─→ query_rewriter ─┐
                                        ├─→ query_rewriter ─────────────────────────────────────┘                   │
                                        └─→ no_context_answer → END  ◀──────────── (retry_count >= MAX_RETRIES) ─────┘
                                                                                    query_rewriter → retrieve_summary (루프)
```

2단계 게이트로 구성된다: **1차(저비용) 필터** `route_after_hits`가 검색 히트 자체가 없거나 `MIN_SCORE` 미만이면 LLM 호출 없이 바로 재시도/포기를 결정하고, **2차(심층) 필터** `confidence_checker`가 실제 컨텍스트를 구성한 뒤 RAGAS 스타일 LLM 평가로 confidence_score를 계산해 재시도/포기를 결정한다. 두 게이트 모두 재시도가 필요하면 `query_rewriter`가 검색어를 LLM으로 재작성하고 `retrieve_summary`로 되돌아간다(`MAX_RETRIES`, 기본 2회). 재시도 소진 시 `generate_answer`를 호출하지 않고 즉시 안내 문구로 종료한다.
...
#### ③ `route_after_hits` ([nodes.py](app/graph/nodes.py))
- `summary_hits + chunk_hits`를 합친 뒤 최댓값 점수가 `MIN_SCORE`(기본 0.2) 이상이면 `build_context`로, 아니면 재시도/포기 판단(`_retry_or_give_up`)으로 라우팅.
- 검색 히트가 아예 없거나 점수가 낮으면 컨텍스트를 만들지 않고 곧바로 재시도 여부를 결정 — RAGAS LLM 호출(비용) 자체를 아낀다.

#### ④ `build_context` ([nodes.py](app/graph/nodes.py), `make_build_context(db)` 클로저)
...
#### ⑤ `confidence_checker` ([nodes.py](app/graph/nodes.py))
- RAGAS 스타일 reference-free 평가(PRD_v2 4장 산식을 단순화 적용): `doc_scores`의 최댓값을 `similarity_score`(0~1 클램프)로 삼고, `llm.evaluate_context(original_question, context)`로 LLM이 `context_precision`/`context_recall`을 각각 0~1로 심사한다.
  - `ragas_score = (context_precision + context_recall) / 2`
  - `confidence_score = similarity_score × 0.4 + ragas_score × 0.6`
- `context`가 비어 있으면(①의 필터를 통과했지만 build_context 결과가 빈 경우는 없지만 방어적으로) LLM 호출 없이 `confidence_score=0.0` 반환.
- `route_after_confidence`가 `confidence_score >= CONFIDENCE_THRESHOLD`(기본 0.5)면 `generate_answer`로, 미달이면 재시도/포기 판단으로 라우팅.

#### ⑥ `query_rewriter` ([nodes.py](app/graph/nodes.py)) — 재시도 전용
- `llm.rewrite_query(original_question, question)`로 검색어를 LLM이 한 문장으로 재작성(모호한 표현 구체화, 도메인 무관 질의는 원문 그대로 반환하도록 프롬프트에 명시).
- `retry_count`를 1 증가시키고 `question`을 재작성된 검색어로 교체 → `retrieve_summary`로 루프. `original_question`은 불변으로 유지되어 최종 답변 생성에 계속 쓰인다.
- `_retry_or_give_up`이 `retry_count < MAX_RETRIES`일 때만 이 노드로 라우팅하며, 소진되면 `no_context_answer`로 즉시 종료해 불필요한 LLM 호출을 아낀다(PRD_v2 11장과 동일한 트레이드오프).

#### ⑦ `generate_answer_node` ([nodes.py](app/graph/nodes.py))
- `confidence_checker` 통과 시에만 실행(그래프 조건부 엣지로 보장). Azure OpenAI Chat Completions 1회 호출(`llm.generate_answer`). 시스템 프롬프트는 하드코딩된 한 문장: "컨텍스트에 있는 내용만 근거로 답하고, 없는 내용은 추측하지 말고 모른다고 답해라."
- 검색에는 재작성된 `question`을 쓰지만, 최종 답변은 `original_question` 기준으로 생성(PRD_v2와 동일한 설계).
- `history`(멀티턴 이력)를 메시지에 포함.

#### ⑧ `extract_sources` ([nodes.py](app/graph/nodes.py))
- 로깅만 수행, 상태 변경 없음.
...
class AssistantState(TypedDict, total=False):
    ...
    retry_count: int                   # query_rewriter가 증가시킴, 초기값 0
    similarity_score: float            # doc_scores 최댓값 (0~1)
    context_precision: float           # RAGAS LLM 평가 결과
    context_recall: float              # RAGAS LLM 평가 결과
    ragas_score: float                 # (context_precision + context_recall) / 2
    confidence_score: float            # similarity_score×0.4 + ragas_score×0.6
```

PRD_v2의 `retrieved_docs`/`reranked_docs` (재랭킹 전용 필드)에 대응하는 필드는 아직 없다 — 재랭킹 노드는 로드맵 3단계로 남아 있다(6장 참고).
...
- Qdrant 연결 실패 → 502 `QDRANT_UNAVAILABLE`, LLM 호출 실패(임베딩/채팅/RAGAS 평가/쿼리 재작성 공통) → 502 `LLM_API_ERROR`, Postgres 실패 → 502 `DB_UNAVAILABLE` ([app/errors.py](app/errors.py), [nodes.py](app/graph/nodes.py)).
- 그래프 호출 시 `config={"recursion_limit": 50}`을 명시한다 — 재시도 루프로 `retrieve_summary`가 최대 `MAX_RETRIES+1`회 반복 실행될 수 있어 LangGraph 기본 재귀 한도(25)보다 넉넉히 잡는다.
...
| `MIN_SCORE` | 0.2 | 1차(저비용) 게이트 임계값 — 검색 히트 자체가 없거나 너무 낮으면 RAGAS 평가 없이 재시도/포기 (실제 Qdrant 점수 분포 기준 튜닝 필요) |
| `CONFIDENCE_THRESHOLD` | 0.5 | 2차(심층) 게이트 임계값 — `confidence_score` 미달 시 재시도/포기 (실 데이터 기준 튜닝 필요) |
| `MAX_RETRIES` | 2 | `query_rewriter` 최대 재시도 횟수 |

Gemini 프로바이더, `RERANK_TOP_N` 등 PRD_v2의 나머지 설정 항목은 아직 존재하지 않는다(재랭킹 노드 미구현, 6장 참고).
...
- **재랭킹 없음**: `doc_scores` 병합은 Qdrant 원점수 그대로 쓴다. entity/keyword/최신성 가중치를 더하는 재랭킹 노드는 아직 없다(question_analyzer 부재로 entity/keyword 자체가 없음) — 로드맵 3단계.
...
| 재랭킹 | 벡터 유사도+entity+keyword+최신성 가중합 | 없음 — Qdrant 점수 그대로 사용 (로드맵 3단계) |
| 신뢰도 게이트 | RAGAS(context_precision/recall) + similarity 가중합, threshold 미달 시 재시도 | ✅ **구현됨** — 동일한 가중합(`similarity×0.4 + ragas×0.6`) 방식을 1차(MIN_SCORE)+2차(CONFIDENCE_THRESHOLD) 2단계 게이트로 적용(`confidence_checker`) |
| 재시도 | 최대 3회, 기법별 쿼리 재작성(rewriting → expansion → multi-query 3단계) | ✅ **구현됨(단순화)** — 최대 `MAX_RETRIES`(기본 2)회, 단일 기법(LLM 쿼리 재작성)만 사용, 단계별 기법 전환 없음(`query_rewriter`) |
...
1. ~~**신뢰도 게이트 고도화**~~ ✅ **완료**: `MIN_SCORE`(1차, 저비용) + `confidence_checker`의 RAGAS 스타일 LLM 평가(2차, `context_precision`/`context_recall` → `confidence_score`)로 2단계 게이트를 구성했다([nodes.py](app/graph/nodes.py), [llm.py](app/clients/llm.py)의 `evaluate_context`).
2. ~~**재시도 루프**~~ ✅ **완료**: `query_rewriter`가 신뢰도 미달 시 LLM으로 검색어를 재작성해 `retrieve_summary`로 되돌아간다(`MAX_RETRIES`, 기본 2회). PRD_v2의 3단계 기법 전환(rewriting→expansion→multi-query)까지는 아니고 단일 재작성 기법만 사용 — 필요 시 추가 확장 여지로 남겨둔다.
...
1~2단계 구현으로 추가된 LLM 호출 비용: 요청당 최소 1회(`evaluate_context`, 컨텍스트가 있을 때)가 추가되고, 재시도가 발생하면 회당 `evaluate_context` 1회 + `rewrite_query` 1회가 더해진다. 검색 히트가 아예 없는 경우는 `evaluate_context` 호출 없이 곧바로 재시도/포기를 판단해 비용을 아낀다.
